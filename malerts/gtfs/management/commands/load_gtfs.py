    ...
    def _load_routes(self, root_dir):  
        ...
        def create_cmd(line): 
            (route, created) = Route.objects.get_or_create(route_id=check_field(line, 'route_id'), 
                                    short_name=check_field(line, 'route_short_name', True),
                                    long_name=check_field(line, 'route_long_name',True), 
...
def check_field(reader, field, optional=False):
    if field in reader and reader[field]:
        ...
        raise CommandError("Field %s was empty or non-present in file %s" %(field,reader) )
    else:
        return None
